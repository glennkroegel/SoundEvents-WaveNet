...
def train(loader, model, optimizer, scheduler, scaler):
    '''training loop'''
    props = {'loss': 0, 'lr': 0}
    model.train()
    for i, batch in enumerate(loader):
...
def main():
    ...
    model = Model(channels=[2, 8], depth=6, num_classes=num_classes).to(device)
    print(model)
    ...
    for epoch in tqdm(range(epochs)):
        train_props = train(train_iter, model, optimizer, scheduler, scaler)
